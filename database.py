...
class Database:
            ...
            try:
                with sqlite3.connect(self.db_path) as db:
                    ...
                    query = f"""
                        ...
                        FROM chats WHERE chat_id = ? AND is_active = 1
                    """
    ...
    async def deactivate_chat(self, chat_id: int) -> bool:
        """Деактивация чата (бот был удален)"""
        def _deactivate_chat_sync():
            try:
                with sqlite3.connect(self.db_path) as db:
                    db.execute("UPDATE chats SET is_active = 0 WHERE chat_id = ?", (chat_id,))
                    db.commit()
                    return True
            except Exception as e:
        ...
        def _assign_moderator_sync():
            try:
                with sqlite3.connect(self.db_path) as db:
                    ...
                    from bot import DEFAULT_RANK_PERMISSIONS
    ...
    async def has_permission(self, chat_id: int, user_id: int, permission_type: str) -> Optional[bool]:
        ...
        def _has_permission_sync():
            try:
                with sqlite3.connect(self.db_path) as db:
                    # Получаем ранг пользователя из БД
                    cursor = db.execute("""
                        SELECT rank FROM chat_moderators 
                        WHERE chat_id = ? AND user_id = ?
                    """, (chat_id, user_id))
                    ...
                    if not row:
                        # Если нет ранга в БД, возвращаем None (будет использован fallback)
                        return None
        ...
        return await asyncio.get_event_loop().run_in_executor(None, _has_permission_sync)
        ...
        try:
            ...
            for user_id in inactive_users:
                try:
                    ...
                    await friends_db.delete_user_friends(user_id)
